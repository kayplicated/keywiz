...
mod tests {
    ...
    fn roundtrip_through_json() {
        ...
        let parsed: StatsFile = serde_json::from_str(&json).unwrap();

        assert_eq!(parsed.version, CURRENT_VERSION);
        assert_eq!(parsed.layout, "test");
        assert_eq!(parsed.stats.get('a').unwrap().attempts, 2);
        assert_eq!(parsed.stats.get('a').unwrap().correct, 1);
        assert_eq!(parsed.stats.get('b').unwrap().attempts, 1);
    }

    #[test]
    fn flatten_produces_no_nested_keys_wrapper() {
        let mut s = Stats::new();
        s.record('a', true);
        ...
        // With #[serde(flatten)] the `keys` field lives at the top level.
        // Without it, we'd see `"keys":{"keys":...}`.
        assert!(!json.contains(r#""keys":{"keys""#), "unexpected nesting: {json}");
    }
}
